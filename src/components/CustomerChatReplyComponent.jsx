...
const CustomerChatReplyComponent = ({text, sender, animate, timeStamp, username, imageUrl, bgColor}) => {
    const {adminDetails} = useContext(AdminPanelContext)
    return <div className={`relative ${animate ? 'animate-slide-up' : ''} px-2 mx-8 flex ${sender == 'admin' ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className={`absolute bottom-0 ${sender == 'admin' ? '-right-5.5 w-6 h-6 ' + adminDetails.bgColor : '-left-5.5 w-6 h-6 ' + bgColor} flex justify-center items-center text-xs text-white rounded-full `}>{sender == 'admin' ? adminDetails.imageUrl ? <img className="rounded-full w-6 h-6" src="https://img.freepik.com/premium-photo/adult-man-serene-face-expression-studio-portrait_53876-75419.jpg?uid=R197307981&ga=GA1.1.1783492781.1737620913&semt=ais_hybrid&w=740" /> : adminDetails.name.at(0).toUpperCase() : imageUrl ? <img className="rounded-full w-6 h-6" src={imageUrl} /> : username.at(0).toUpperCase()}</div>
    </div>
}
